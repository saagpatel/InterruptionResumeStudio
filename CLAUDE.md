...
Native macOS desktop app (Tauri 2.0 + React + TypeScript + SQLite) that captures cognitive work context via global hotkey overlay, menu bar, or manual entry — and surfaces a structured resume card on return. No cloud, no accounts, 100% local.
...
- State: Zustand 5.x; Date/Time: date-fns 3.x; Icons: Lucide React 0.561.x
...
- TypeScript strict mode — type with `unknown` + narrowing; avoid `!` non-null assertions without a comment
...
- Scope to IMPLEMENTATION-ROADMAP.md phases — no out-of-phase feature additions
- All persistence is SQLite via Rust commands; localStorage/sessionStorage/any browser storage is off-limits
- Overlay window is pre-loaded at startup — show/hide it on hotkey; never create/destroy per press
- Network requests are disabled — enforce via `"http": { "all": false }` in Tauri capabilities config
- All user data must stay under `~/Library/Application Support/interruption-resume-studio/`
...
Prior lessons, decisions, and context for this project live in SecondBrain at `wiki/maps/projects/interruption-resume-studio.md`. The whole vault is searchable via the `engraph` MCP — query it for this project + its stack before non-trivial work.
